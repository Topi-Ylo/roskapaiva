-- Cleanup pass for the modal_body text seeded by 0007. The original 0007
-- shipped with em dashes used as sentence-structure punctuation, which is
-- against the project's writing style. Bolt.new auto-applied 0007 against
-- the database before the source could be cleaned, so this migration rewrites
-- all six services' modal_body rows with the corrected copy.
--
-- Unlike 0007, this migration updates unconditionally (no `where modal_body
-- is null` guard) so it always overwrites whatever the row currently holds.
--
-- Safe to re-run.
...
update public.services
  set modal_body = $$Yritysyhteistyö Roskapäivän kanssa on enemmän kuin kertaluonteinen tapahtuma. Se on pitkäjänteinen kumppanuus ympäristöteon ympärillä, ja räätälöin yhteistyön juuri teidän tarpeisiinne sopivaksi.
...
update public.services
  set modal_body = $$Roskapäivän sosiaalisen median kanavat (Instagram @roskapaiva ja TikTok) tavoittavat aktiivisen, ympäristötietoisen yleisön ympäri Suomen. Seuraajat ovat sitoutuneita ja arvostavat aitoa, vastuullista sisältöä.
...
update public.services
  set modal_body = $$Tarjoan kouluille ja oppilaitoksille valmiita ja räätälöitäviä oppimateriaaleja luonnon roskaantumisesta, kiertotaloudesta ja kestävistä elämäntavoista. Materiaalit on suunniteltu sopimaan opetussuunnitelmaan ja toimimaan inspiroivana lähtökohtana ympäristökeskusteluille.
