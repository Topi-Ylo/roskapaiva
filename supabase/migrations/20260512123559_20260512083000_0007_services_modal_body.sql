...
-- Seed three new services. image_url is intentionally NULL — the user uploads
-- proper imagery via /admin/services. The card and modal both render a styled
-- placeholder (large numeral on the forest gradient) when image_url is empty.
...
Pyydä mediakortti, niin lähetän tarkemmat tiedot tavoittavuudesta, hinnoittelusta ja yhteistyömahdollisuuksista.$$,
  ...
  (
    ...
    $$Tarjoan kouluille ja oppilaitoksille valmiita ja räätälöitäviä oppimateriaaleja luonnon roskaantumisesta, kiertotaloudesta ja kestävistä elämäntavoista. Materiaalit on suunnitelty sopimaan opetussuunnitelmaan ja toimimaan inspiroivana lähtökohtana ympäristökeskusteluille.
