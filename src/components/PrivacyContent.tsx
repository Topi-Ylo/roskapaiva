...
/**
 * Tietosuojaseloste — the GDPR Article 13 notice, as shared markup so the page
 * at /tietosuoja and the overlay opened from the form cannot drift apart.
 *
 ...
 * version, not through a text box at midnight. LAST_UPDATED must be bumped
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="border-t border-cream/10 py-7">
      <h3 className="font-display text-xl text-cream md:text-2xl">{title}</h3>
...
export default function PrivacyContent() {
  ...
  return (
    <>
      ...
      <p className="mt-4 text-sm text-cream/45">Päivitetty {LAST_UPDATED}</p>
      <p className="mt-6 text-base leading-relaxed text-cream/75">
        Tämä seloste kertoo, mitä henkilötietoja Roskapäivä kerää, mihin niitä käytetään ja mitkä
        oikeudet sinulla on. Selosteen mukainen käsittely koskee ennen kaikkea
      ...
      <div className="mt-10">
        <Section title="Rekisterinpitäjä">
          <p>
            {CONTROLLER.name} (Y-tunnus {CONTROLLER.registryId})
          </p>
          <p>
