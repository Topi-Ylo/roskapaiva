...
const EMPTY: FormState = {
  ...
  is_large: false, is_wide: false, object_position: 'center',
...
export default function TimelineAdmin() {
  ...
  const refresh = async () => {
    ...
    const { data, error } = await supabase
      .from('timeline_entries').select('*').order('sort_order');
    if (error) setError(error.message);
    setItems((data ?? []) as TimelineEntry[]);
    setLoading(false);
  };
  useEffect(() => { refresh(); }, []);
  ...
  const startEdit = (e: TimelineEntry) => {
    ...
    setForm({
      ...
      image_url: e.image_url ?? '', is_large: e.is_large, is_wide: e.is_wide,
      object_position: e.object_position, sort_order: e.sort_order,
  ...
  const onSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const payload = {
      year: form.year.trim(), title: form.title.trim(),
      description: form.description.trim() || null,
      image_url: form.image_url.trim() || null,
      is_large: form.is_large, is_wide: form.is_wide,
  ...
  const onDelete = async (e: TimelineEntry) => {
    if (!supabase) return;
    if (!confirm(`Poistetaanko ${e.year} – ${e.title}?`)) return;
    const { error } = await supabase.from('timeline_entries').delete().eq('id', e.id);
    if (error) { alert(error.message); return; }
    if (editingId === e.id) { setEditingId(null); setForm(EMPTY); }
    await refresh();
  };

  return (
    <div className="mx-auto max-w-5xl">
      <AdminPageHeader eyebrow="Sisältö" title="Aikajana"
        actions={editingId ? <GhostButton onClick={startNew}>Uusi merkintä</GhostButton> : null} />

      <form onSubmit={onSubmit} className="rounded-lg border border-cream/10 bg-forest-deep p-6 md:p-8">
        <p className="font-display text-xl text-cream">{editingId ? 'Muokkaa merkintää' : 'Uusi merkintä'}</p>
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          ...
          <div className="space-y-2 self-end">
            ...
            <label className="flex items-center gap-3"><input type="checkbox" checked={form.published} onChange={(e) => setForm({ ...form, published: e.target.checked })} className="h-4 w-4 accent-amber" /><span className="text-sm text-cream/80">Julkaistu</span></label>
          </div>
        </div>
      ...
      <div className="mt-12">
        <p className="font-display text-xl text-cream">Merkinnät ({items.length})</p>
        {loading ? <p className="mt-4 text-cream/60">Ladataan…</p> : items.length === 0 ? <p className="mt-4 text-cream/60">Ei vielä merkintöjä.</p> : (
          <ul className="mt-4 space-y-3">
            {items.map((e) => (
              <li key={e.id} className="flex flex-col gap-4 rounded-lg border border-cream/10 bg-forest-deep p-4 md:flex-row md:items-center">
                <div className="h-20 w-32 flex-shrink-0 overflow-hidden rounded bg-forest-night">
                  {e.image_url ? <img src={e.image_url} alt="" className="h-full w-full object-cover" loading="lazy" /> : <div className="flex h-full w-full items-center justify-center text-xs text-cream/40">Ei kuvaa</div>}
                </div>
                <div className="flex-1">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {e.is_large && <span className="rounded-full bg-amber/15 px-2 py-0.5 text-[10px] uppercase tracking-widest text-amber">tall</span>}
                    {e.is_wide && <span className="rounded-full bg-amber/15 px-2 py-0.5 text-[10px] uppercase tracking-widest text-amber">wide</span>}
                    {!e.published && <span className="rounded-full bg-cream/10 px-2 py-0.5 text-[10px] uppercase tracking-widest text-cream/55">luonnos</span>}
                  </div>
                  {e.description && <p className="mt-1 text-sm text-cream/70">{e.description}</p>}
                  <p className="mt-1 text-xs text-cream/40">Järjestys: {e.sort_order} · {e.object_position}</p>
                </div>
                <div className="flex gap-2"><GhostButton onClick={() => startEdit(e)}>Muokkaa</GhostButton><DangerButton onClick={() => onDelete(e)}>Poista</DangerButton></div>
              </li>
            ))}
          </ul>
        )}
